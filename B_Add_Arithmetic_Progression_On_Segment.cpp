...
struct update
{
    int a = 0, d = 0; //  a = first term, d = common difference
    update() {}
    update(pair<int, int> val)
    {
        a = val.first, d = val.second;
    }
...
struct segtree
{
    ...
    void pushdown(const int32_t &v, const int32_t &tl, const int32_t &tr)
    {
        ...
        apply(v << 1, tl, tm, u[v]);
        update temp = u[v];
        temp.a += (tm - tl + 1) * temp.d;
        apply(v << 1 | 1, tm + 1, tr, temp);
